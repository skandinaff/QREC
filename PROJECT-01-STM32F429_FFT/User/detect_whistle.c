...
void DetectWhistle(void) {
    ...
    if ((freq >= WHISTLE_FREQ_RANGE_LOW && freq <= WHISTLE_FREQ_RANGE_HIGH)) {  // Whistle withing range
      TIM_Cmd(TIM2, ENABLE);
			GPIO_ResetBits(ONBOARD_LED_GPIO, ONBOARD_LED_3);
			GPIO_SetBits(ONBOARD_LED_GPIO, ONBOARD_LED_4);
    }
    if (freq < WHISTLE_FREQ_RANGE_LOW) {  // Cutting off frequencies less than
			GPIO_SetBits(ONBOARD_LED_GPIO, ONBOARD_LED_3);
			GPIO_ResetBits(ONBOARD_LED_GPIO, ONBOARD_LED_4);
			TIM_Cmd(TIM2, DISABLE);
    }
		if (freq > WHISTLE_FREQ_RANGE_HIGH) { // Cutting off frequencies more than 2400
			GPIO_SetBits(ONBOARD_LED_GPIO, ONBOARD_LED_3);
			GPIO_ResetBits(ONBOARD_LED_GPIO, ONBOARD_LED_4);
			TIM_Cmd(TIM2, DISABLE);
    }
...
void DetectClapByEnergy(void) {
	// THIS IS A PROTOTYPE TODO : Decide if this is needed at all
    FFT_OUT_t in;
    in = ComputeFFT();
...
void SilenceDetectionByEnergy(void) {
    ...
    in = ComputeFFT();
		...
		if(N >= SIL_AVG_SAMPLES && silence_thresh_is_set == false){
			setSilenceThresh( (silence_thresh_avg/SIL_AVG_SAMPLES) + correction_value );  // Correction vaule sets Threshold a little bit higher than ambient loudness
			//if(getSilenceThresh() > SIL_THR_MAX) setSilenceThresh(SIL_THR_MAX); // TODO: decide if this is needed
		...
		if (getSecondCount() > SILENCE_TIME) { 
				__enable_irq();   // This is impornant so that UART continue to work properly
				TIM_Cmd(TIM2, DISABLE);
...
void DrawBar(uint16_t bottomX, uint16_t bottomY, uint16_t maxHeight, uint16_t maxValue, float32_t value, uint16_t foreground,
        uint16_t background) {
					// THIS IS ONLY FOR DESCOVERY BOARD TODO: REMOVE
    uint16_t height;
